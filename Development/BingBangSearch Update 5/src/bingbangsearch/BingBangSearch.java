...
public class BingBangSearch {
	...
	public static void main(String[] args) throws SAXException, IOException, ParserConfigurationException {
		...
		for(int g = 0; g < lastResults.size();g++)
		{
			...
			System.out.println(lastResults.get(g).getFrequency());
		}
    	...
    	{	
    		if(engine == "bing")
    		{
				...
				for (Element link : links) 
				{
					...
					{
						if(!(temp.trim().contains("go.microsoft")||link.text().trim().contains("Ajutor")||temp.trim().contains("onlinehelp.microsoft.com")))
						{
				...
				{	
					...
					if(temp.startsWith("/url?q="))
					{
						if(!(temp.trim().contains("googleusercontent")
								...
								||link.text().trim().contains("În cache")
